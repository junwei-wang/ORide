...
// A test of the protocol with one rider and malicious drivers (who try to
// scramble other drivers' data).
template <typename T, size_t Degree, size_t NbModuli, typename U>
void test_oride_impl(int count)
{
    ...
    for (int i = 0 ; i < count ; ++i)
    {
        ...
        // SP pre-computes masks.
        auto t0_mask_ntt = std::chrono::high_resolution_clock::now();
        for (size_t i = 0 ; i < Degree ; ++i)
        {
            cc[i] = 1;
            PT mask(cc);
            mask.invntt();
            masks.emplace_back(CT2::to_cipher_poly(mask));
            cc[i] = 0;
        }
        auto t1_mask_ntt = std::chrono::high_resolution_clock::now();
        ...
        for (size_t i = 0 ; i < Degree ; ++i)
        {
            cc[i] = c[i];
            dd[i] = d[i];
            // Each driver is malicious and sets a random value somewhere.
            cc[(i+1)%Degree] = distribution(generator);
            dd[(i+1)%Degree] = distribution(generator);
            ...
            // masking them first.
            auto t0_mask = std::chrono::high_resolution_clock::now();
            edix *= masks[i];
            ediy *= masks[i];

            edx += edix;
            edy += ediy;
            auto t1_mask = std::chrono::high_resolution_clock::now();

            mask.add(std::chrono::duration_cast<std::chrono::microseconds>(t1_mask - t0_mask).count());
        }
        ...
        mask_ntt.add(std::chrono::duration_cast<std::chrono::microseconds>(t1_mask_ntt - t0_mask_ntt).count());
    ...
    std::cerr << "Mask NTT (startup) : " << mask_ntt << " us/" << Degree << std::endl;
    std::cerr << "Mask : " << mask << " us/driver" << std::endl;
...
void test_oride()
{
    ...
    int count = 30;
    test_oride_impl<uint64_t, 4096, 2, uint32_t>(count);
}

int main()
{
    std::cerr << "******************** Malicious drivers ********************" << std::endl;
    test_oride();
    return 0;
}
